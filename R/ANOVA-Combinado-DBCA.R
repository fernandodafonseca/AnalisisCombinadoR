...
#' @param nombre_bloque El nombre de la columna que contiene la variable de bloque.
#' @param nombre_var_resp El nombre de la columna que contiene la variable respuesta.
#' @param nombre_tratamiento El nombre de la columna que contiene la variable de tratamiento.
#' @param nombre_entornos El nombre de la columna que contiene la variable de entorno.
#' @param tipo_varianza Un entero que indica el tipo de estructura de varianza. 1 para homogeneidad y 2 para heterogeneidad.
#' @param efecto_bloque Un entero que indica el tipo de efecto del bloque en el modelo. 1 para aleatorio y 2 para fijo.
#'
...
analisis_combinado_DBCA <- function(datos, nombre_bloque, nombre_var_resp, nombre_tratamiento, nombre_entornos, tipo_varianza, efecto_bloque) {
  ...
  if (tipo_varianza == 1) {
    ...
    } else if (efecto_bloque == 2) {
    modelo <- gls(var_resp ~ tratamiento + entorno + bloque + entorno:tratamiento, data = datos)
    }else {
  ...
  row.names(anova_resultados) <- c("",nombre_tratamiento,nombre_entornos,nombre_interaccion)
    ...
    nombre_interaccion <- paste(nombre_tratamiento,nombre_entornos, sep=":")
    row.names(anova_resultados) <- c("",nombre_tratamiento,nombre_entornos,nombre_bloque,nombre_interaccion)
  }else {
    stop("El tipo de efecto para el bloque no es valido. Debe ser 1 (aleatorio) o 2 (fijo).")
  }
